...
def proxy_media(url: str = Query(...)):
    resp = requests.get(url, headers=HEADERS, impersonate="chrome120", stream=True)
    if resp.status_code != 200:
...
@app.get("/api/download/android-file")
def download_android_file(img_url: str = Query(...), vid_url: str = Query(...), filename: str = "livephoto.jpg"):
    ...
    vid_resp = requests.get(vid_url, headers=HEADERS, impersonate="chrome120")
